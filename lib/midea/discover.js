...
function parseV2OrV3(msg, sourceAddress, protocol) {
    ...
    // Need byte at 74+ssidLen for firmware version, 63+5 for MAC.
    if (data.length < 75 + ssidLen) return null;
        ...
        socket.on("message", (msg, info) => {
            ...
            try {
                ...
                if (dev && !found.has(dev.id)) {
                    ...
                    log.debug(`discover: ${info.address} reply did not parse (first 32B: ${msg.subarray(0, Math.min(32, msg.length)).toString("hex")})`);
                }
            } catch (err) {
