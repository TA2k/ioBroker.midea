...
function discover(opts = {}) {
    ...
    const targets = explicitTargets || enumerateBroadcastTargets();
    ...
    return new Promise((resolve) => {
        ...
        socket.on("message", (msg, info) => {
            // Silly-level raw-packet trace so users can confirm packets are
            // arriving at all (helps tell "no broadcast got through" apart
            // from "broadcast worked but nothing parsed").
            if (log.silly) log.silly(`discover: rx ${msg.length}B from ${info.address}:${info.port}`);
            try {
                ...
                for (const port of ports) {
                    socket.send(BROADCAST, 0, BROADCAST.length, port, target, (err) => {
                        if (err) {
                            ...
                            log.silly(`discover: tx ${BROADCAST.length}B to ${target}:${port}`);
                        }
                    });
                }
            }
