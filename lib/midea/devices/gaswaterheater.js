...
class GasWaterHeaterDevice extends BaseDevice {
    ...
    async setStatus(updates = {}) {
        ...
        // New-protocol single-property set (body_type 0x14, [tag, value, 0..]).
        // Maps directly to MessageNewProtocolSet in midea-local e3/message.py.
        const newProto = {
            zeroColdWater: (v) => [0x03, v ? 0x01 : 0x00],
            zeroColdPulse: (v) => [0x04, v ? 0x01 : 0x00],
            smartVolume: (v) => [0x07, v ? 0x01 : 0x00],
            targetTemperature: (v) => [0x08, Number(v) & 0xFF],
        };
        for (const key of Object.keys(newProto)) {
            if (updates[key] !== undefined) {
                const [tag, val] = newProto[key](updates[key]);
                const tail = Buffer.alloc(17, 0x00);
                const body = Buffer.concat([Buffer.from([0x14, tag, val]), tail]);
                const cmd = createCommand(body, {
                    msgType: 0x02,
                    protocol: 0x03,
                    applianceType: APPLIANCE_TYPE_GAS_WATER_HEATER,
                ...
                this.log.debug(`E3[${this.id}]: setNewProto ${key}=${updates[key]} -> tag=0x${tag.toString(16)}`);
                const reply = await this._request(cmd, `setE3New_${key}`);
                try {
                    const inner = reply.subarray(10, reply.length - 1);
                    this._mergeStatus(parseE3Body(inner));
                } catch (err) {
                    this.log.debug(`E3[${this.id}]: setNewProto reply parse skipped (${err.message})`);
                }
                return this.status;
            }
        }
        ...
        const zeroCold = fld("zeroColdWater") ? 0x01 : 0x00;
        const protection = fld("protection") ? 0x08 : 0x00;
        const zeroColdPulse = fld("zeroColdPulse") ? 0x10 : 0x00;
        const smartVolume = fld("smartVolume") ? 0x20 : 0x00;
        const target = updates.targetTemperature !== undefined
            ? Number(updates.targetTemperature) & 0xFF
            : (cur.targetTemperature || 40) & 0xFF;
