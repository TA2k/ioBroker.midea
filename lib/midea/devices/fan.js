...
class FanDevice extends BaseDevice {
    ...
    async refreshStatus() {
        ...
        const parsed = parseFAResponse(inner);
    ...
    async setStatus(updates = {}) {
        ...
        if (updates.fanSpeed !== undefined) {
            ...
            s.fanSpeed = v;
            if (v > 0 && !s.powerOn) s.powerOn = true;
        }
        ...
        try {
            ...
            this.log.debug(`FA[${this.id}]: setStatus reply parse skipped (${err.message})`);
        }
        return this.status;
    }
    ...
    /**
     * Build the FA set body. midea-local uses an 18- or 49-byte payload
     * depending on subtype; we always emit the long form unless subtype is
     * within 1..0x0A (small variant).
     */
    _buildSetBody(s) {
        const small = this.subtype >= 1 && (this.subtype <= 0x0A || this.subtype === 0xA1);
        const body = Buffer.alloc(small ? 19 : 49);
        ...
        if (!small) {
            body[13 + 1] = 0xFF; // mirrors midea-local non-small variant comment
        } else if (this.subtype !== 0x0A) {
            body[13 + 1] = 0xFF;
        }
