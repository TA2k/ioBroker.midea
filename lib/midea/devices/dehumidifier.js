...
// Mode-byte → name mapping. Names are taken from the cloud-served Lua
// plugin (T_0000_A1_*.lua: VALUE_MODE_*) so they match what the Midea
// cloud and the official app use:
//   1=set, 2=continuity, 3=auto, 4=dry_clothes, 5=dry_shoes, 8=fan
// midea-local uses different (human-readable) labels but the same codes.
const MODE_BY_NAME = {
    set: 1, continuity: 2, auto: 3, dry_clothes: 4, dry_shoes: 5, fan: 8,
    // Backwards-compatible aliases for users who configured names from older
    // releases (pre-2026-05). Map each old label to the same numeric code.
    setpoint: 1, continuous: 2, smart: 3, dryer: 4,
};
const MODE_BY_INDEX = {
    1: "set", 2: "continuity", 3: "auto", 4: "dry_clothes", 5: "dry_shoes", 8: "fan",
};
