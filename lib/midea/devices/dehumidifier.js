...
// plugin (T_0000_A1_*.lua, VALUE_MODE_* table). The generator script
// scripts/generate-maps.js extracts the same values into
// scripts/generated/a1.js for diff-tracking only — runtime carries the
// values inline so the library is fully self-contained without a build
// step.
const MODE_BY_INDEX = { 1: "set", 2: "continuity", 3: "auto", 4: "dry_clothes", 5: "dry_shoes", 8: "fan" };
const MODE_BY_NAME = { set: 1, continuity: 2, auto: 3, dry_clothes: 4, dry_shoes: 5, fan: 8 };

const FAN_BY_NAME = { SILENT: 40, LOW: 60, MEDIUM: 70, HIGH: 80, AUTO: 102, silent: 40, low: 60, medium: 70, high: 80, auto: 102 };
...
class DehumidifierDevice extends BaseDevice {
    ...
    async setStatus(updates = {}) {
        ...
        // midea-local naming aliases (commit 91c3dde): `pump` and `pumpEnable`
        // map to the same bits as pumpSwitch / pumpSwitchFlag.
        if (updates.pump !== undefined) status.pumpSwitch = !!updates.pump;
        if (updates.pumpEnable !== undefined) status.pumpSwitchFlag = !!updates.pumpEnable;
