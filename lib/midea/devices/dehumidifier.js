...
// Mode-byte → name mappings. Canonical source is the cloud-served Lua
// plugin (T_0000_A1_*.lua, VALUE_MODE_* table); a generator extracts it
// into generated/a1-maps.js. We try to load that file first so a fresh
// regeneration just propagates here, but fall back to a hand-coded copy
// of the same values when the generated module is missing — happens when
// this folder is lifted out of the repo without the generator output.
//   1=set, 2=continuity, 3=auto, 4=dry_clothes, 5=dry_shoes, 8=fan
const FALLBACK_MODE_BY_INDEX = { 1: "set", 2: "continuity", 3: "auto", 4: "dry_clothes", 5: "dry_shoes", 8: "fan" };
let a1maps;
try {
    a1maps = require("../generated/a1-maps");
} catch (_e) {
    a1maps = {
        MODE_BY_INDEX: FALLBACK_MODE_BY_INDEX,
        MODE_BY_NAME: { set: 1, continuity: 2, auto: 3, dry_clothes: 4, dry_shoes: 5, fan: 8 },
    };
}
// Drop the 0=invalid sentinel — it's a "no mode set" marker, never reported.
const MODE_BY_INDEX = { ...a1maps.MODE_BY_INDEX };
delete MODE_BY_INDEX[0];
const MODE_BY_NAME = { ...a1maps.MODE_BY_NAME };
delete MODE_BY_NAME.invalid;
...
class DehumidifierDevice extends BaseDevice {
    ...
    async refreshStatus() {
        ...
        this.log.debug(`DH[${this.id}]: refreshStatus() <- bytes=${reply.length}`);
    ...
    async setStatus(updates = {}) {
        ...
        if (updates.childLock !== undefined) status.childLock = !!updates.childLock;
    ...
    /**
     * Decode a 0xAA frame from this device. The 0xA1 dehumidifier emits
     * the same body layout for query reply, set reply and notify push,
     * so all three sources funnel through this single decoder.
     *
    ...
    _processFrame(frame, _ctx) {
        ...
        try {
            ...
            return this._mergeStatus(this._normalizeStatus(status));
        } catch (err) {
            this.log.debug(`DH[${this.id}]: _processFrame failed (${err && err.message})`);
            return undefined;
        }
    }
