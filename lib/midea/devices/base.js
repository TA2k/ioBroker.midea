...
// A transient command failure shouldn't flip the device offline while the
// TCP session is still visibly receiving frames (late replies, heartbeats,
// pushes). Only demote to offline when nothing has arrived for this long.
...
class BaseDevice extends EventEmitter {
    ...
    _ensureLan() {
        ...
        this._lan.on("activity", () => this._setOnline(true, "LAN activity"));
        return this._lan;
    }

    /**
     * Update the online flag and emit a change event only on transition.
     * Keeps the ioBroker `info.online` state in sync without redundant writes.
     *
     * @param {boolean} value
     * @param {string} [reason]
     */
     ...
     * @param {string} label
     * @param {object} [opts]
     * @param {boolean} [opts.noRetry=false] skip the transient-error retry —
     *   useful for capability-detection sweeps where a 10 s timeout × N
     *   blocks the poll cycle for too long. Real commands keep the retry
     *   so a brief app-collision doesn't drop a user-issued setStatus.
     */
    async _request(cmd, label, opts = {}) {
        ...
        for (;;) {
            try {
                ...
                // Keep the device online if the session is still receiving
                // frames — a single command can time out (firmware answering
                // late) while the LAN link is otherwise perfectly alive.
                ...
                if (recentInbound) {
                    this.log.debug(`Device[${this.id}]: _request(${label}) transient failure but session still receiving frames; keeping online=true`);
                } else {
    ...
    _mergeStatus(parsed) {
        ...
        for (const key of Object.keys(parsed)) {
            // A field that wasn't in the cache yet must always go through,
            // even if its first observed value equals the eventual cached
            // value. Otherwise json2iob never gets a chance to publish the
            // initial value — the state object is created on schema setup
            // but never written to.
            const isNew = !Object.prototype.hasOwnProperty.call(this.status, key);
            if (isNew || this.status[key] !== parsed[key]) {
