...
class ElectricWaterHeaterDevice extends BaseDevice {
    ...
    async setStatus(updates = {}) {
        ...
        const protection = updates.protection ? 0x04 : 0x00;
        const wholeTank = updates.wholeTankHeating === undefined
            ? (this.status.wholeTankHeating ? 0x02 : 0x01)
            : (updates.wholeTankHeating ? 0x02 : 0x01);
        const target = updates.targetTemperature !== undefined
            ? Number(updates.targetTemperature) & 0xFF
            : (this.status.targetTemperature || 50) & 0xFF;
        const variable = updates.variableHeating ? 0x10 : 0x00;
