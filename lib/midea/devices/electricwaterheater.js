...
function parseE2Body(body) {
    ...
    // E2 "memory" (Memo U in the MSmart app): smart memory boost mode.
    // midea-local commit d9c1f14, source T_0000_E2_24.lua.
    if (body.length > 23) out.memory = (body[23] & 0x08) > 0;
    if (body.length > 25) {
...
class ElectricWaterHeaterDevice extends BaseDevice {
    ...
    async setStatus(updates = {}) {
        ...
        // E2 memory ("Memo U") uses a different new-protocol layout
        // than the byte12/byte13 TLV family below: body_type 0x03 with
        // the flag on byte 3 (midea-local message.py:108, commit d9c1f14).
        if (updates.memory !== undefined) {
            const value = updates.memory ? 0x80 : 0x00;
            const body = Buffer.from([0x03, 0x00, 0x00, value, 0x00, 0x00, 0x00, 0x00, 0x00]);
            const cmd = createCommand(body, {
                msgType: 0x02,
                protocol: 0x03,
                applianceType: APPLIANCE_TYPE_ELECTRIC_WATER_HEATER,
            ...
            this.log.debug(`E2[${this.id}]: setMemory(${updates.memory})`);
            const reply = await this._request(cmd, "setE2Memory");
