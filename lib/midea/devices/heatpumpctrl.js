...
function parseC3UnitPara(body, off) {
    ...
    // instant_power0 (current heat-pump power draw in watts) lives in
    // bytes 82-83 of the UnitPara payload — added in midea-local 6.6.0
    // (commit 8cef9f4). Useful as a real-time wattmeter without having
    // to wait for the 15-min totals.
    if (body.length >= off + 84) {
        out.instantPower = (body[off + 82] << 8) | body[off + 83];
    }
    return out;
}
