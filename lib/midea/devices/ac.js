...
class ACDevice extends BaseDevice {
    ...
    async refreshHumidity() {
        ...
        this._processFrame(reply, { source: "query", msgType: reply[9] });
        // C1/0x45 also delivers outdoor fan RPM and the defrost flag —
        // even on units without a humidity sensor. Treat the query as
        // supported when ANY of those fields came back, otherwise the
        // outdoor-fan state never updates after the first sweep.
        const has = (k) => this.status[k] !== undefined && this.status[k] !== null;
        if (!has("indoorHumidity") && !has("outdoorFanSpeedRpm") && !has("defrostActive")) {
