...
class ACDevice extends BaseDevice {
    ...
    /**
     * Convenience: fetch all extended telemetry groups (1, 2, 5, 7) in
     * sequence. Each group that times out gets blacklisted so unsupported
     * firmware doesn't waste a slot every poll.
     */
    async refreshExtendedTelemetry() {
        await this.refreshGroup(1, "getGroup1");
        await this.refreshGroup(2, "getGroup2");
        await this.refreshGroup(7, "getGroup7");
        return this.status;
    }
