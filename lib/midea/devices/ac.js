...
class ACDevice extends BaseDevice {
     ...
     */
    async refreshGroup(group, label) {
        ...
        let reply;
        try {
            reply = await this._request(cmd, lbl);
        } catch (err) {
    ...
    /**
     * Fetch every C1 sub-body the firmware accepts. midea-local#424 reports
     * non-zero data in groups 1, 2, 3, 5, 6, 7, 11 and 12 on a real
     * appliance, but the parsable layout is only documented for 1, 2, 5
     * and 7 (msmart-ng TurboLed fork). The remaining groups still get
     * queried — unknown sub-bodies fall through parseC1 without
     * producing fields but the device's reply confirms the group is
     * supported (and lets a future parser pick it up).
     *
     * Groups 4 (energy) and 5 (humidity) are intentionally skipped here:
     * they have their own refresh methods (refreshPowerUsage /
     * refreshHumidity) that are already wired into the main poll cycle.
     *
     * Each group that times out is blacklisted via _unsupportedQueries
     * so the next sweep doesn't burn 4 s on it again.
     */
    async refreshExtendedTelemetry() {
        for (let g = 0; g <= 15; g++) {
            if (g === 4 || g === 5) continue;
            await this.refreshGroup(g, `getGroup${g}`);
        }
        return this.status;
    }
