...
class ACDevice extends BaseDevice {
    ...
    async _sweepGroupCapabilities() {
        ...
        for (let g = 0; g <= 15; g++) {
            if (g === 4 || g === 5) continue;
            await this.refreshGroup(g, `getGroup${g}`, { noRetry: true });
        }
    }
    ...
    async setNewProtocol(updates = {}) {
        ...
        if (updates.freshAir !== undefined) {
            packs.push(packNewProtocolValue(NEW_PROTOCOL_TAGS.fresh_air, Buffer.from([updates.freshAir ? 0x01 : 0x00])));
            count++;
        }
