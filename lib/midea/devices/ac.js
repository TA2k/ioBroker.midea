...
class ACDevice extends BaseDevice {
    ...
    async refreshStatus() {
        ...
        if (parsed.type === 0xBB) {
            // V2 subprotocol firmware. We don't decode this body yet — log
            // once per device so the user can open an issue with the raw
            // bytes for support to add proper handling.
            if (!this._loggedSubprotocol) {
    ...
    async refreshPowerUsage() {
        ...
        /** @type {any} */
        const parsed = parseFrame(reply, this._powerAnalysisMethod);
        this.log.debug(`AC[${this.id}]: refreshPowerUsage() <- type=0x${(parsed.type || 0).toString(16)}`);
        if (parsed.type !== 0xC1) {
            this._unsupportedQueries.add("powerUsage");
            return null;
        }
        const merged = {};
        for (const k of ["powerUsage", "totalEnergyConsumption", "totalOperatingConsumption", "currentEnergyConsumption", "realtimePower"]) {
            if (typeof parsed[k] === "number") merged[k] = parsed[k];
        }
        if (Object.keys(merged).length) this._mergeStatus(merged);
        return merged.powerUsage !== undefined ? merged.powerUsage : null;
    }
    ...
    async refreshOperatingTime() {
        ...
        const body = Buffer.from([0x41, 0x21, 0x01, 0x40, 0x00, 0x01]);
        const cmd = createCommand(body, {
            msgType: 0x03,
            protocol: 0x03,
            applianceType: APPLIANCE_TYPE_AC,
        ...
        /** @type {any} */
        const parsed = parseFrame(reply, this._powerAnalysisMethod);
        if (parsed.type !== 0xC1 || typeof parsed.electrifyTime !== "number") {
            this._unsupportedQueries.add("operatingTime");
            return null;
        }
        const merged = {
            electrifyTime: parsed.electrifyTime,
            totalOperatingTime: parsed.totalOperatingTime,
            currentOperatingTime: parsed.currentOperatingTime,
        };
        this._mergeStatus(merged);
        return merged;
    }
    ...
    async refreshHumidity() {
        ...
        const body = Buffer.from([0x41, 0x21, 0x01, 0x45, 0x00, 0x01]);
        const cmd = createCommand(body, {
            msgType: 0x03,
            protocol: 0x03,
            applianceType: APPLIANCE_TYPE_AC,
        ...
        /** @type {any} */
        const parsed = parseFrame(reply, this._powerAnalysisMethod);
        if (parsed.type !== 0xC1 || typeof parsed.indoorHumidity !== "number") {
            this._unsupportedQueries.add("humidity");
            return null;
        }
        this._mergeStatus({ indoorHumidity: parsed.indoorHumidity });
        return parsed.indoorHumidity;
    }
    ...
    async refreshNewProtocol() {
        ...
        /** @type {any} */
        const parsed = parseFrame(reply);
        if (parsed.type !== 0xB1 || !parsed.status) {
            this._unsupportedQueries.add("newProtocol");
            return null;
        }
        this._mergeStatus(parsed.status);
    ...
    async setNewProtocol(updates = {}) {
        ...
        /** @type {any} */
        const parsed = parseFrame(reply);
        this.log.debug(`AC[${this.id}]: setNewProtocol() <- type=0x${(parsed.type || 0).toString(16)}`);
