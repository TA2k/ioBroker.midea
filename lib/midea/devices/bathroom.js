...
class LightDevice extends BaseDevice {
    ...
    _processFrame(frame, _ctx) {
        ...
        if (inner[0] === 0xA4) return this._mergeStatus(parse13Body(inner));
        return undefined;
    }
...
class BathroomHeaterDevice extends BaseDevice {
    ...
    _processFrame(frame, _ctx) {
        if (!frame || frame.length < 12) return undefined;
        const inner = frame.subarray(10, frame.length - 1);
        if (inner[0] === 0x01) return this._mergeStatus(parse26Body(inner));
        return undefined;
    }
...
class DishwasherX34Device extends BaseDevice {
    ...
    _processFrame(frame, _ctx) {
        if (!frame || frame.length < 12) return undefined;
        const inner = frame.subarray(10, frame.length - 1);
        return this._mergeStatus(parse34Body(inner));
    }
    ...
    async setStatus(updates = {}) {
        if (updates.powerOn !== undefined) {
            ...
            const reply = await this._request(cmd, "setPower");
            this._processFrame(reply, { source: "set", msgType: reply[9] });
            return this.status;
        }
        if (updates.childLock !== undefined) {
            ...
            const reply = await this._request(cmd, "setLock");
            this._processFrame(reply, { source: "set", msgType: reply[9] });
            return this.status;
        }
        return this.status;
...
class BathroomFanDevice extends BaseDevice {
    ...
    _processFrame(frame, _ctx) {
        if (!frame || frame.length < 12) return undefined;
        const inner = frame.subarray(10, frame.length - 1);
        if (inner[0] === 0x01) return this._mergeStatus(parse40Body(inner));
        return undefined;
    }
