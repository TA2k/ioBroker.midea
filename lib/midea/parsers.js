    ...
    while (i < data.length - 2 && toProcess > 0) {
        ...
        if (tag === 0x00 && len > 0) {
            switch (id) {
                ...
                // msmart-ng commit 7ad92d0: split out 0x93/0x94 as
                // aux fan speed capabilities (were "EMERGENT_HEAT_WIND" /
                // "HEAT_PTC_WIND" before).
                case 0x93: caps.auxFanSpeed = value === 1; break;
                case 0x94: caps.auxHeatFanSpeed = value === 1; break;
                // msmart-ng commit b120d62: Fresh Air capability.
                case 0x4B: caps.freshAir = value === 1; break;
                default: break;
            }
        }
        ...
        if (tag === 0x02 && len > 0) {
            switch (id) {
                case 0x10:
                    caps.fanSpeedControl = value !== 1;
                    // msmart-ng commit 6c7fc1a: value 9 adds silent/auto.
                    caps.fanSilent = value === 6 || value === 9;
                    caps.fanLow = [3, 4, 5, 6, 7, 9].includes(value);
                    caps.fanMedium = [5, 6, 7].includes(value);
                    caps.fanHigh = [3, 4, 5, 6, 7, 9].includes(value);
                    caps.fanAuto = [4, 5, 6, 9].includes(value);
                    caps.fanCustom = value === 1;
                    break;
                case 0x12:
                ...
                case 0x14:
                    // msmart-ng commit bcd329d: expanded mode tables. Older
                    // narrow lists (value 0..3 only) missed newer firmware
                    // that ships values 4..15.
                    caps.heatMode = [1, 2, 4, 6, 7, 9, 10, 11, 12, 13].includes(value);
                    caps.coolMode = [0, 1, 3, 4, 5, 6, 7, 8, 9, 11, 13, 14, 15].includes(value);
                    caps.dryMode = [0, 1, 5, 6, 9, 11, 13, 14, 15].includes(value);
                    caps.autoMode = [0, 1, 2, 7, 8, 9, 13, 14].includes(value);
                    caps.auxHeatMode = value === 9; // Heat & Aux
                    caps.auxMode = [9, 10, 11, 13, 14, 15].includes(value); // Aux only
                    break;
                case 0x15:
...
function parseC1(data, analysisMethod) {
    ...
    if (sub === 0x44 && data.length >= 19) {
        ...
        // Some PortaSplit firmware reports the binary energy fields in
        // 0.1 kWh units rather than 1 kWh. Expose a pre-scaled variant
        // so users can pick whichever matches the SmartHome app
        // (realtimePowerBinary is already in W and stays untouched).
        out.totalEnergyConsumptionBinaryKwh = out.totalEnergyConsumptionBinary / 10;
        out.currentEnergyConsumptionBinaryKwh = out.currentEnergyConsumptionBinary / 10;
    ...
    // that doesn't include a sub-body type). Only fires when the reply has
    // no sub-body byte at all — otherwise an unknown group (e.g. 0x4B) with
    // a 19+ byte payload would get mis-decoded as `powerUsage`.
    if (sub === null && data.length >= 19) {
        let n = 0;
        let m = 1;
        for (let i = 0; i < 3; i++) {
            n += (data[18 - i] & 0x0F) * m;
...
const NEW_PROTOCOL_TAGS = {
    ...
    // NOTE: msmart-ng PropertyId.FRESH_AIR = 0x004B is the same ID as
    // fresh_air_2 above — we keep the existing freshAir_2 alias for
    // backwards compatibility and don't add a second mapping.
