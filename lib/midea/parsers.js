...
function parseC1(data, analysisMethod) {
    ...
    if (sub === 0x45 && data.length >= 5) {
        ...
        // midea-msmart issue #248 + midea-local#424: a non-zero byte 8 in
        // sub-body 0x45 (typically 0x61 during normal operation) flips to
        // 0 while the unit is in defrost / off-heating, plus a few other
        // states. Surface a best-effort flag — caller can use it as a
        // hint, but it's not a strict defrost-only signal.
        if (data.length > 8) out.heatingActive = data[8] !== 0;
        return out;
    }
