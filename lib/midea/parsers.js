...
// midea-local PowerFormats: 1=BCD, 2=BINARY (0.1 kWh), 3=MIXED (byte = 0..99), 12=BINARY1
const POWER_FORMATS = { BCD: 1, BINARY: 2, MIXED: 3, BINARY1: 12 };
...
function parsePower(method, bytes) { return _parsePowerValue(method, bytes) / 10; }
...
function parseC1(data, analysisMethod) {
    const out = {};
    const method = analysisMethod === undefined ? POWER_FORMATS.MIXED : analysisMethod;
    const sub = data.length >= 4 ? data[3] : null;

    if (sub === 0x44 && data.length >= 19) {
        out.totalEnergyConsumption = parseConsumption(method, data.subarray(4, 8));
        out.totalOperatingConsumption = parseConsumption(method, data.subarray(8, 12));
        out.currentEnergyConsumption = parseConsumption(method, data.subarray(12, 16));
        out.realtimePower = parsePower(method, data.subarray(16, 19));
...
function parseFrame(frame, powerAnalysisMethod) {
    ...
    switch (body[0]) {
        ...
        case 0xC1:
            return { type: 0xC1, ...parseC1(body, powerAnalysisMethod) };
        default:
