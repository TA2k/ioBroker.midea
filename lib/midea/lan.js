    ...
    _ensureConnected() {
        ...
        return /** @type {Promise<void>} */ (new Promise((resolve, reject) => {
            ...
            const onTimeout = () => {
                ...
                reject(
                    new Error(
                        `LanClient: connect timeout to ${this.host}:${this.port} — the appliance is reachable on UDP but TCP/6444 is being blocked, or another LAN client (the phone app) is currently connected. Only one TCP control session is allowed at a time.`,
                    ),
                );
            };
            const cleanup = () => {
            ...
            this._socket.write(frame, (err) => {
                ...
                this._cmdTimer = setTimeout(() => {
                    if (this._pendingReject) {
                        ...
                        r(
                            new Error(
                                `LanClient: timeout waiting for ${label} — TCP/6444 is being blocked, or another LAN client (the phone app) is currently connected. Only one TCP control session is allowed at a time.`,
                            ),
                        );
                    }
                }, COMMAND_TIMEOUT_MS);
