...
const TCP_PORT = 6444;
// midea-local uses a 10 s connect socket timeout (device.py SOCKET_TIMEOUT);
// 5 s gave up too early on congested Wi-Fi / slow-booting appliances and
// marked the device offline where midea-local would have connected.
...
// Force a reconnect after this many consecutive command timeouts on one
// session. keepSocket lets a single late reply land, but a socket that keeps
// timing out is a zombie — reconnecting re-takes the LAN slot cleanly.
// midea-local reconnects after 12 recv timeouts (device.py).
const MAX_CONSECUTIVE_TIMEOUTS = 12;
...
class LanClient extends EventEmitter {
    constructor(opts) {
        ...
        // Consecutive command timeouts on the current session. We keep the
        // socket open after a single timeout so a late reply can still land
        // (err.keepSocket), but a socket that keeps timing out is dead — force
        // a reconnect after this many in a row. midea-local reconnects after
        // 12 recv timeouts (device.py MAX_RETRIES × SOCKET_TIMEOUT ≈ 120 s).
    ...
    async _pump() {
        ...
        try {
            ...
            if (err && err.keepSocket && this._consecutiveTimeouts < MAX_CONSECUTIVE_TIMEOUTS) {
                this._consecutiveTimeouts += 1;
                this.log.debug(`LanClient[${this.deviceId}]: keeping socket open after ${item && item.label} timeout for late replies (${this._consecutiveTimeouts}/${MAX_CONSECUTIVE_TIMEOUTS})`);
                this._armIdleClose();
            } else {
                if (err && err.keepSocket) {
                    this.log.debug(`LanClient[${this.deviceId}]: ${this._consecutiveTimeouts} consecutive timeouts — session is dead, forcing reconnect`);
                }
                this._destroySocket();
            }
    ...
    async _send(item) {
        ...
        // A bare "ERROR" frame on the command path means the appliance
        // rejected the session (midea-local device.py:464 treats it as a
        // reconnect trigger). Tear the socket down so the next request
        // re-authenticates instead of decrypting garbage.
        if (reply.length === 5 && reply.toString() === "ERROR") {
            this._inFlight = null;
            this._consecutiveTimeouts = 0;
            item.reject(new Error("LanClient: device returned ERROR on command — session rejected"));
            this._destroySocket();
            setImmediate(() => this._pump());
            return;
        }
        ...
        this._inFlight = null;
        this._consecutiveTimeouts = 0; // a real reply proves the session is alive
        item.resolve(body);
        this._lastActivity = Date.now();
        this._armIdleClose();
        setImmediate(() => this._pump());
    ...
    _destroySocket() {
        ...
        this._tcpKey = null;
        this._socket = null;
        this._consecutiveTimeouts = 0; // fresh session next connect
        this._rxBuf = Buffer.alloc(0);
        if (this._cmdTimer) clearTimeout(this._cmdTimer);
        this._cmdTimer = null;
        this._stopHeartbeat();
