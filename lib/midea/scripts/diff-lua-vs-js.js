...
const FILE_BY_TYPE = {
    ...
    E6: "gasboiler.js",
    ED: "waterpurifier.js",
    FA: null, // no device class yet — listed so the coverage row still shows
    FC: "purifier.js",
};
...
function main() {
    ...
    for (const typeHex of Object.keys(FILE_BY_TYPE)) {
        ...
        summaryRows.push({
            ...
            exposedKeys: (snap.exposedKeys || []).length,
            acceptedValues: Object.keys(snap.acceptedValues || {}).length,
            encodeBytes: Object.keys(snap.encodeBytes || {}).length,
        });
    }

    console.log(`\n--- Plugin coverage (cloud snapshot side) ---`);
    console.log("type   modes fans keys vals byts strs body  expK accV encB");
    for (const r of summaryRows) {
        console.log(
            ...
            String(r.exposedKeys).padStart(4) + " " +
            String(r.acceptedValues).padStart(4) + " " +
            String(r.encodeBytes).padStart(4)
        );
    }
