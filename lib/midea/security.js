...
function addHeader0x5A5A(encryptedPayload, messageId, deviceId) {
    ...
    packet.writeUInt32LE(messageId >>> 0, 8);
...
/**
 * Compute the udpId used by the V3 cloud when retrieving the per-device
 * token/key pair: SHA256 of the device id (8-byte little-endian), then XOR
 * the two 16-byte halves.
 *
 * @param {string|Buffer|number} deviceId
 * @returns {string} 32-char hex
 */
function deriveUdpId(deviceId) {
    let idBuf;
    if (Buffer.isBuffer(deviceId)) {
        idBuf = Buffer.from(deviceId);
    } else {
        let idHex;
        if (typeof deviceId === "number") {
            ...
            idHex = String(deviceId);
            if (/^\d+$/.test(idHex)) idHex = BigInt(idHex).toString(16);
        }
        if (idHex.length % 2 === 1) idHex = "0" + idHex;
    ...
    for (let i = 0; i < 6; i++) reversed[i] = idBuf[idBuf.length - 1 - i];
    const digest = sha256(reversed);
