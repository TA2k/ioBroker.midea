...
function addHeader0x5A5A(encryptedPayload, messageId, deviceId) {
    ...
    // Bytes [20..27]: device id as 8-byte little-endian integer.
    // midea-local writes `device_id.to_bytes(8, "little")` in
    // packet_builder.py:71. We previously truncated to 6 bytes (msmart-ng
    // layout) which left bytes [26..27] zero — some firmware silently drop
    // the frame when those high bytes are missing.
    let idHex;
