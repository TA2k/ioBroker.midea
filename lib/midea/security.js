...
/**
 * Write the 8-byte little-endian device id into bytes [20..27] of a 5A5A
 * packet. midea-local packet_builder.py:71 does `device_id.to_bytes(8,
 * "little")`. Accepts decimal string, hex string, number or Buffer.
 *
 * @param {Buffer} packet 40-byte 5A5A header (mutated in place)
 * @param {string|Buffer|number} deviceId
 */
function writeDeviceIdLE(packet, deviceId) {
    ...
    // idBuf is big-endian; write little-endian into [20..27].
    for (let i = 0; i < 8; i++) {
...
/**
 * Write the UTC date/time into bytes [12..19] of a 5A5A packet, one decimal
 * field per byte. midea-local packet_builder.py builds this from
 * `datetime.now(UTC).strftime("%Y%m%d%H%M%S%f")[:16]` and reverses the
 * two-digit groups — so the subsecond byte is HUNDREDTHS (0..99), NOT raw
 * milliseconds. Writing `ms & 0xff` (up to 255) put an out-of-range value
 * there and some strict firmware silently dropped the frame.
 *
 * @param {Buffer} packet 40-byte 5A5A header (mutated in place)
 */
...
/**
 * Build a 5A5A heartbeat packet (message type 0x10, body 0x7b) to keep the
 * device's single LAN-client slot warm between sparse polls. Mirrors
 * midea-local's periodic keepalive. Returns the packet with the MD5
 * checksum already appended; the caller wraps it in 0x8370 for V3.
 *
 * @param {string|Buffer|number} deviceId
 * @returns {Buffer}
 */
...
function decode0x8370(data, tcpKey) {
    ...
    // midea-local decodes BOTH encrypted msgtypes (0x03 response and 0x06
    // request) the same way (security.py:353). Some firmware tags its reply
    // with 0x06; decoding only 0x03 left that body still AES-CBC encrypted
    // and the downstream AES-ECB step then produced garbage / threw.
