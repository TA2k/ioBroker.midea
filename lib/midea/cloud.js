        ...
        for (const m of methods) {
            ...
            try {
                data = await this._apiRequest("/v1/iot/secure/getToken", {
                    ...
                    reqId: crypto.randomBytes(16).toString("hex"),
                    udpid: udpId,
                    // The MSmartHome cloud rejects getToken with 3004
                    // "value is illegal" unless `applianceCodes` is also
                    // sent. midea-local cloud.py:273 (post-v6.8.0). Other
                    // clouds ignore the extra field.
                    applianceCodes: String(deviceId),
                });
            } catch (err) {
            ...
            if (matched) {
                // midea-local cloud.py:284 lowercases token/key before
                // returning. Hex case is firmware-dependent on the cloud
                // side; some MAS regions return mixed-case for the same
                // pair across calls, which then breaks downstream string
                // comparisons. Normalize early.
                const token = String(matched.token || "").toLowerCase();
                const key = String(matched.key || "").toLowerCase();
                candidates.push({ method: m.name, token, key });
                this.log.debug(`getToken[${deviceId}]: ${m.name} udpId=${udpId} received successfully — token=${token.slice(0, 8)}…, key=${key.slice(0, 8)}…`);
            } else {
...
class CloudClientV1 {
    ...
    async getTokenCandidates(deviceId) {
        ...
        for (const m of methods) {
            ...
            let result;
            try {
                result = await this._apiRequest("/v1/iot/secure/getToken", {
                    udpid: udpId,
                    applianceCodes: String(deviceId),
                });
            } catch (err) {
            ...
            if (matched) {
                ...
                const key = String(matched.key || "").toLowerCase();
                candidates.push({ method: m.name, token, key });
                this.log.debug(`getToken[${deviceId}] (V1): ${m.name} udpId=${udpId} received successfully — token=${token.slice(0, 8)}…, key=${key.slice(0, 8)}…`);
            } else {
