...
class CloudClientV1 {
    ...
    async getToken(udpId) {
        await this.authenticate();
        this.log.debug(`CloudClientV1: getToken(udpId=${udpId})`);
        const result = await this._apiRequest("/v1/iot/secure/getToken", { udpid: udpId });
        const list = result.tokenlist || [];
        for (const pair of list) {
            if (pair.udpId === udpId) return { token: pair.token, key: pair.key };
        }
        throw new Error(`CloudClientV1: no token/key pair found for udpId=${udpId}`);
    }
