    ...
    async getTokenCandidates(deviceId) {
        ...
        for (const m of methods) {
            ...
            try {
                ...
                this.log.debug(`CloudClient: getToken(deviceId=${deviceId}, method=${m.name}) failed: ${err && err.message ? err.message : err} — trying next method`);
                continue;
            }
            const list = (data.data && data.data.tokenlist) || [];
            stats.push(`${m.name}=${list.length}`);
            this.log.debug(`CloudClient: getToken(deviceId=${deviceId}, method=${m.name}) tokenlist=${list.length} entries (matching udpId=${udpId})`);
            for (const pair of list) {
                if (pair.udpId === udpId) {
                    candidates.push({ method: m.name, token: pair.token, key: pair.key });
                    this.log.debug(`CloudClient: getToken(deviceId=${deviceId}, method=${m.name}) token=${pair.token} key=${pair.key}`);
                    break;
                }
            }
        }
        this.log.debug(`CloudClient: getTokenCandidates(deviceId=${deviceId}) -> ${candidates.length} pair(s) [${stats.join(", ")}]`);
    ...
    async getTokenCandidates(deviceId) {
        ...
        for (const m of methods) {
            ...
            try {
                ...
                stats.push(`${m.name}=err`);
                errors.push(`${m.name}: ${err && err.message ? err.message : err}`);
                this.log.debug(`CloudClientV1: getToken(deviceId=${deviceId}, method=${m.name}) failed: ${err && err.message ? err.message : err} — trying next method`);
                continue;
            }
            ...
            for (const pair of list) {
                if (pair.udpId === udpId) {
                    candidates.push({ method: m.name, token: pair.token, key: pair.key });
                    this.log.debug(`CloudClientV1: getToken(deviceId=${deviceId}, method=${m.name}) token=${pair.token} key=${pair.key}`);
                    break;
                }
            }
        }
        this.log.debug(`CloudClientV1: getTokenCandidates(deviceId=${deviceId}) -> ${candidates.length} pair(s) [${stats.join(", ")}]`);
        if (!candidates.length && errors.length === methods.length) {
