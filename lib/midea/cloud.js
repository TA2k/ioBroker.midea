 ...
 * Returns the candidate list (possibly empty if both paths exhausted).
 *
...
async function getTokenCandidatesWithFallback(primary, deviceId, logger) {
    ...
    const fallback = new CloudClientV1({
        appName: "nethome",
    ...
    try {
        ...
        const fbError = err && err.message ? err.message : String(err);
        log.warn(
            `getToken[${deviceId}]: NetHome Plus fallback also failed (${fbError}). ` +
                "If you have your own NetHome Plus / Midea Air / Ariston Clima account, configure it as the cloud app in the adapter settings.",
        );
        return [];
    }
